#warning Temoporary file and a dependent on the RPi4 network interface.
...
#define DDR_MEMORY_END	(__DDR_start+1)
...
static void vInitialiseUncachedMemory( )
{
	 ...
	 */
	pucStartOfMemory = (uint8_t *)( ( ( uintptr_t )pucStartOfMemory + UNCACHED_MEMORY_SIZE ) & ( ~( UNCACHED_MEMORY_SIZE - 1 ) ) );

	if( ( ( uintptr_t )pucStartOfMemory ) + UNCACHED_MEMORY_SIZE > DDR_MEMORY_END )
	{
	...
	else
	{
		 ...
		 */
		Bcm_SetTlbAttributes( ( uintptr_t )pucStartOfMemory, 0xc02 ); // addr, attr
