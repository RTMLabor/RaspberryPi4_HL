...
extern void ssiHandler(HTTPClient_t *pxClient, char* ssi, unsigned int length);
...
BaseType_t xRc = 0;
	...
	if( pxClient->bits.bReplySent == pdFALSE_UNSIGNED )
	{
		...
		pxClient->pxParent->pcExtraContents[0] = '\0';
	...
	if( xRc >= 0 ) do
	{
		...
		if( uxCount > 0u )
		{
			if( uxCount > (sizeof( pxClient->pxParent->pcFileBuffer )-1) )
			{
				uxCount = (sizeof( pxClient->pxParent->pcFileBuffer )-1); //change to pcFileBufferSize-1 because string needs to be null terminated to use strstr
			}
			ff_fread( pxClient->pxParent->pcFileBuffer, 1, uxCount, pxClient->pxFileHandle );
			pxClient->pxParent->pcFileBuffer[uxCount] = 0; // null terminate string
			...
			char* leadOut;
			xRc = 0;
			char* bufferPointer = pxClient->pxParent->pcFileBuffer;
			leadIn = strstr(bufferPointer, g_pcTagLeadIn);
			leadOut  = strstr(bufferPointer, g_pcTagLeadOut);
			unsigned int bytesFromFileSent = 0;
			if(leadIn == NULL)
			{
			...
			else
			{
				while(leadIn!=NULL)
				{
					...
					if(leadOut==NULL)
					{
						bufferPointer = leadIn+LEN_TAG_LEAD_IN;
						continue;
					}

					unsigned int newLength = leadIn - bufferPointer;
					xRc = FreeRTOS_send( pxClient->xSocket, bufferPointer, newLength, 0 );
					if( xRc < 0 )
					{
						break;
					}								
					bytesFromFileSent += newLength;
					bytesFromFileSent += leadOut+LEN_TAG_LEAD_OUT-leadIn;
					bufferPointer = (leadOut + LEN_TAG_LEAD_OUT);
					ssiHandler(pxClient, leadIn+LEN_TAG_LEAD_IN, leadOut-leadIn-LEN_TAG_LEAD_IN);
					FreeRTOS_send( pxClient->xSocket, pxClient->ssiBuffer, pxClient->ssiLength, 0 );
					leadIn = strstr(bufferPointer, g_pcTagLeadIn);

				}


				xRc = FreeRTOS_send( pxClient->xSocket, leadOut+LEN_TAG_LEAD_OUT, uxCount-bytesFromFileSent, 0 );
...
char pcSlash[ 2 ];
	...
	#if( ipconfigHTTP_HAS_HANDLE_REQUEST_HOOK != 0 )
	{
		...
		size_t xResult;
			...
			char* httpRequestCharacterLocation = strchr( pxClient->pcUrlData, '?' );
			*httpRequestCharacterLocation  = '\0';
			if( xResult > 0 )
			{
	...
	if(pxClient->pcUrlData[ 1 ] == '\0'||pxClient->pcUrlData[ 0 ] == '\0')
	{
